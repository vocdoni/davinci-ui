...
type MiniApp = Record<string, string | string[]>

/**
 * Build the Farcaster miniapp data object
 * Used by both the manifest file and HTML meta tags
 */
export function buildFarcasterData(appDomain: string): MiniApp {
  const miniapp: MiniApp = {
    version: '1',
    name: process.env.APP_TITLE || 'DAVINCI',
    iconUrl: `https://${appDomain}/icons/ic_launcher_google_play.png`,
    homeUrl: `https://${appDomain}`,
    imageUrl: `https://${appDomain}/icons/ic_launcher_google_play.png`,
    subtitle: 'Anonymous anti-coercion voting',
    description: 'Decentralized voting platform powered by DAVINCI protocol',
    splashImageUrl: `https://${appDomain}/light/android-chrome-512x512.png`,
    splashBackgroundColor: '#F5F1E8',
    primaryCategory: 'social',
    tags: ['governance', 'voting', 'anti-coercion', 'dao', 'community'],
  }
 ...
 */
export function buildFarcasterEmbedTag(miniapp: MiniApp) {
  return {
    version: miniapp.version,
    imageUrl: miniapp.imageUrl,
    button: {
      title: miniapp.buttonTitle || 'Create Vote',
      action: {
        type: 'launch_miniapp',
        name: miniapp.name,
        url: miniapp.homeUrl,
